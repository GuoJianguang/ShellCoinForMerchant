...
- (void)didScroll
{
    ...
    if (!_defersItemViewLoading || fabsf([self minScrollDistanceFromOffset:_lastUpdateOffset toOffset:_scrollOffset]) >= 1.0f)
    {
        ...
        {
            [AutoScroller shareAutoScroller].temp = self.currentItemIndex;
...
- (void)startAnimation
{
    if (!_timer)
    {
        self.timer = [NSTimer timerWithTimeInterval:1./60
                                             target:self
...
-(void)startScroll {
//    __weak typeof(SwipeView*) weak_self = self;
//    dispatch_async(dispatch_get_main_queue(), ^{
//        [weak_self.scrollTimer invalidate];
//        weak_self.scrollTimer = [NSTimer scheduledTimerWithTimeInterval:5.f target:weak_self selector:@selector(autoScroll:) userInfo:nil repeats:YES];
//    });
}
...
-(void)autoScroll:(NSTimer*)timer {
    ...
    [self scrollToItemAtIndex:currentPage duration:0.0];
    if (toRight) {
        currentPage++;
    }else {
        currentPage--;
